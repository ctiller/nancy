...
Previously, the Repository Explorer UI in the dashboard utilized a server `CheckoutGitBranch` endpoint. Selecting a branch via the web interface would execute `git checkout <branch>` natively on the host filesystem. This had catastrophic consequences on active runtime testing, causing merge conflicts or blowing up concurrent orchestration sessions by shifting the working tree asynchronously.

## Decision

We have fundamentally refactored the Repository Explorer sub-systems to read natively from the Git object database using `git2` without modifying the physical working tree.

We established the `get_repo_tree_ssr` and `read_file_text_ssr` methods inside `web/src/repo.rs`, which directly invoke `repo.revparse_single(&branch)`, peeling to commits, extracting the specific `git2::Tree`, and drilling down natively until reading raw blob contents `into_blob()`.

The UI reactivity system (via `leptos`) now preserves an active branch selection and streams it back natively over the network context when fetching files.
