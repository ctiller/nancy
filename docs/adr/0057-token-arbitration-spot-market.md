...
- LLM Token requests block until their specific priority bids have been granted from the central coordinator. 
- The Coordinator dynamically maps a `/api/market/task-priority/:task_id` endpoints ensuring Grinders do not waste IO redundantly hydrating the DAG for prioritization.
