...
2. Internal Rust Axum UDS proxy error pipelines gracefully trap `RecvError::Closed` and `SendError` silently instead of panicking upon remote drops.
3. Memory profiles per viewing client compress, avoiding exponential loop recursion faults.
