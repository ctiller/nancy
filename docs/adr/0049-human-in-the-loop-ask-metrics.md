...
Agents automatically enter a long-polling boundary, pausing inference safely until a Human Operator acknowledges the `Ask` node by writing back a `HumanResponse` explicitly addressing that particular `ask_ref`, optionally canceling it.
