...
## Context
As the Nancy orchestrator begins supporting advanced agentic behavior natively, we require a systematic mechanism to dispatch LLM requests. The mechanism must dynamically resolve the underlying Gemini model variants based on request constraints (e.g. structural requirements dictating Gemini 3.1) and intended execution modes (e.g., `fast` loops vs deep `thinking`). Furthermore, it requires emitting compatible OpenAPI standard definitions mapping to natively typed Rust schemas for deterministic inference.
...
- Structural outputs mechanically delegate dynamic constraint schema generation to `schemars` yielding a compatible JSON definition natively supported by `gemini-client-api`.
