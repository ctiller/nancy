...
## Decision
We actively established the `last_update` (Long Polling) UDS pattern inside our `axum` routing configurations.
...
1. The server extracts the `last_update` parameter.
...
- Resolves all synchronization deadlocks inherently. If events fire while a client request is dropped/reconnecting, the underlying version counter de-synchronizes ensuring the client's next loop yields the newest data immediately tracking.
