...
We have replaced the `TokenMarket` with a robust `ArbitrationMarket` in `src/coordinator/market.rs` to manage lease grants through:
...
3. **Trailing Usage Telemetry**: Exact input/output token costs are calculated retroactively across time windows (1m, 3m, 10m, 30m, 100m) via `UsageMetrics`. Model requests eagerly deduct expected lease costs based on their historical rolling averages.
...
- **IPC Protocol Refactor**: The previous `/consumed-tokens` endpoint is deprecated in favor of `/llm-usage` which reports segregated prompt and candidate tokens securely.
