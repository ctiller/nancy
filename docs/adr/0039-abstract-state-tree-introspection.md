...
## Decision
We implement a Dear ImGui inspired abstract serialization graph anchored to active Task Execution loops via `tokio::task_local!`.
...
- It is not possible to share `INTROSPECTION_CTX` generically across disconnected spawned threads safely without propagating `.scope()` references, enforcing disciplined boundary hierarchies securely.
