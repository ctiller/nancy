...
Plumbing the client dynamically into abstractions like traits (`AgentTaskProcessor::process`) and decoupled builders (`LlmBuilder::with_proxy_client`) functionally introduces sprawling structural boilerplate that strictly couples decoupled agent tools to their orchestration layer heavily.

## Decision
We enforce a simple, static singleton design structurally in `src/agent.rs` utilizing the thread-safe `std::sync::OnceLock`. A single global functional initialization block creates exactly one securely bound singleton IPC Unix connection pool scoped for the worker node's operating session:
```rust
...
- Reallocating `reqwest::Client` boundaries across abstractions (such as tools or heavily decoupled LLM evaluation boundaries) strictly enforces `get_coordinator_client...` retrievals gracefully without relying on injected `fn` parameter references dynamically structurally.
