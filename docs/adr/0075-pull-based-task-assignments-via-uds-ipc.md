...
## Context
Previously, nancy operated on a push-based model where the Coordinator actively wrote `CoordinatorAssignmentPayload` events directly to the event ledger whenever a task became ready and a Grinder was online. While conceptually symmetric with the ledger system, it created implicit race conditions mapping assignments, caused unnecessary ledger bloat, and broke boundaries when testing or executing multiple grinders.

As documented in ADR 0030, tasks are fully standalone event payloads. The addition of UDS IPC endpoints (ADR 0031) opened up a clear pathway to transition from push-based assignments (requiring Git serialization and synchronization) to a pull-based memory-tracked model, directly managed in real-time.
...
4. **Local Index Resolution:** Grinders parse the response locally, retrieving the `TaskPayload` via the pre-compiled `LocalIndex` caches, bypassing duplicate fetching.
...
- Mock interactions in testing environments transition from creating fake `CoordinatorAssignmentPayload` structs to simulating real UDS locks.
- The `AppView` `tasks_assigned` boundary no longer relies on pushed payloads.
