...
3. During `PlanTask` execution, the `manager/grinder` orchestrator simulates spinning up an isolated branch (e.g. `refs/heads/nancy/plans/<request_ref>`), writing the multi-document artifacts natively.
...
- Further formalizes standardizing event payloads as pointers, rather than storage blobs natively.
