...
The Axum UI handlers and IPC endpoints now strictly rely on acquiring `shared_identity.write().await` using asynchronous locks, safely abstracting structural JSON mutations within an exclusive in-memory lock without triggering an interrupt. 

Concurrently, we added the cryptographic abstraction `DidOwner::generate()` directly into the schema bindings, consolidating the duplicated Ed25519 identity generation previously fragmented across `init.rs` and `ipc.rs`.
...
- The Axum endpoints safely block identical configuration generation in memory without file-based collisions.
