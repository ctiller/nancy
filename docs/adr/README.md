...
* [0060: LLM Streaming Introspection and Ledger Rollup](0060-llm-streaming-introspection-and-ledger-rollup.md) - Integrates unified Server-Sent Events buffering thoughts to UI graphs safely.
...
* [0068: Execute Task Test Module Migration](0068-execute-task-test-module-migration.md) - Migrates orchestrator workflow integration tests to dynamically embedded modules ensuring deterministic bounds isolating tests successfully.
* [0069: Centralized LLM Gateway Proxy](0069-centralized-llm-gateway-proxy.md) - Transitions LLM external egress entirely to an internal Proxy mapping over Unix Domain Sockets completely dropping Docker bounds to `none` dynamically structurally resolving metrics synchronization internally.
* [0070: Hoisted Persistent HTTP UDS Client](0070-hoisted-persistent-http-uds-client.md) - Replaces dynamic polling reconstructions of the UDS HTTP connection pool with a static global singleton safely across structural abstractions.
* [0072: Sandbox Canonicalization and Gateway Retries](0072-sandbox-canonicalization-and-gateway-retries.md) - Standardizes sandbox environment mapping and gateway retry logic.
* [0071: NanoCent Integer Math](0071-nanocent-integer-math.md) - Replaces floating-point USD calculations with a strongly-typed integer NanoCent mathematically bounded across the system.
* [0073: LLM Token Prediction Models](0073-llm-token-prediction-models.md) - Introduces TaskType classification and `smartcore` KNN/Linear token regression.
* [0074: Leverage External Crates Over Self-Implementation](0074-leverage-external-crates.md) - Policies enforcing the use of established third-party dependencies structurally to minimize testing and maintenance burdens.
* [0075: Pull-Based Task Assignments via UDS IPC](0075-pull-based-task-assignments-via-uds-ipc.md) - Transitions assignment ledger creation gracefully migrating statically binding UDS boundaries via explicit local index resolutions dynamically structurally preserving mapping organically.
...
<!-- IMPLEMENTED_BY: [src/dreamer/mod.rs, web/tests/repo_tests.rs, src/debug/mod.rs, llm-macros/tests/md_test.rs, build.rs] -->
