...
mod tests {
    ...
    async fn test_e2e_real_llm_reviewer_execution() {
        ...
        assert!(result.is_ok(), "Real LLM failed to return a valid structured payload");
        ...
        assert!(!review_output.agree_notes.is_empty(), "LLM failed to populate agree_notes");
        assert!(!review_output.disagree_notes.is_empty(), "LLM failed to populate disagree_notes");
    }
}
