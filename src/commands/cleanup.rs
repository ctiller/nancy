...
pub async fn cleanup<P: AsRef<Path>>(dir: P) -> Result<()> {
    ...
    if gitignore_path.exists() {
        ...
        for line in content.lines() {
            if line.trim() == ".nancy" || line.trim() == ".nancy/" || line.trim() == "/.nancy" || line.trim() == "/.nancy/" {
                changed = true;
            } else {
    ...
    for branch_name in branches_to_delete {
        if let Ok(mut branch) = repo.find_branch(&branch_name, BranchType::Local) {
            ...
            if branch.is_head() {
                tracing::warn!("Cannot firmly delete branch {} because it is currently checked out.", branch_name);
...
mod tests {
    ...
    async fn test_cleanup_removes_dir_and_branches() -> Result<()> {
        ...
        assert!(repo.find_branch("nancy/test-branch", BranchType::Local).is_err());
