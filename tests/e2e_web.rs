...
async fn test_e2e_web_tasks_topology() {
    ...
    let post_res = client
        .post(&format!("https://127.0.0.1:{}/api/tasks", port))
        .header("Content-Type", "application/json")
        .json(&task_payload)
        .send()
        .await
    ...
    let url = format!("https://127.0.0.1:{}/api/tasks/topology", port);
    ...
    while !found && attempts < 50 {
        ...
        if parsed
            .nodes
            .iter()
            .any(|n| n.name == "Test Topology Request")
        {
            assert!(
                parsed.max_width > 0.0,
                "Expected max_width to be populated by backend dugong layout"
            );
            assert!(
                parsed.max_height > 0.0,
                "Expected max_height to be populated by backend dugong layout"
            );
            found = true;
        } else {
            tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;
    ...
    assert!(
        found,
        "Failed to observe Test Topology Request in topology response"
    );
}
...
async fn test_e2e_web_incident_logs() {
    ...
    let did = match &identity {
        nancy::schema::identity_config::Identity::Coordinator { workers, .. } => {
            workers[0].did.clone()
        }
    ...
    writer
        .log_event(nancy::schema::registry::EventPayload::AgentCrashReport(
    ...
    while !found && attempts < 20 {
        let url = format!("https://127.0.0.1:{}/api/incidents/test-crash.log", port);
        if let Ok(res) = client.get(&url).send().await {
            if res.status() == reqwest::StatusCode::OK {
                let text = res.text().await.unwrap();
                assert!(
                    text.contains("explicit panic"),
                    "Log text didn't match. Got: {}",
                    text
                );
                found = true;
            } else {
...
async fn test_e2e_web_tasks_evaluations() {
    ...
    let url = format!("https://127.0.0.1:{}/api/tasks/evaluations", port);
    ...
    while !found && attempts < 150 {
        ...
        let parsed: Vec<schema::TaskEvaluation> =
            serde_json::from_value(payload["evaluations"].clone()).unwrap();
        ...
        if parsed.iter().any(|e| e.score > 0) {
            ...
            if attempts % 10 == 0 {
                println!(
                    "Attempt {}: No evaluations found yet: {:?}",
                    attempts, payload
                );
            }
            tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;
...
async fn test_e2e_web_human_pending_standalone_grinder_hydration() {
    ...
    let res = client
        .get(&format!("{}/api/human/pending", base_url))
        .header("Accept", "application/json")
        .send()
        .await
        .unwrap();
    ...
    let plan_reviews = pending_json["plan_reviews"]
        .as_array()
        .expect("plan_reviews missing or not an array");
...
async fn test_e2e_web_market_state() {
    ...
    let res = client
        .get(&format!("{}/api/market/state", base_url))
        .send()
        .await
        .unwrap();
    ...
    let per_model_stats = market_state["per_model_stats"]
        .as_array()
        .expect("Expected per_model_stats array in deserialized payload structurally");
    assert!(
        !per_model_stats.is_empty(),
